// 🌈 THEME MANAGER - Centralized Theme Management (Fixed)
...
function applyTheme(themeName, source = 'local') {
  const theme = window.themes[themeName] || Object.values(window.themes).find(t => t.isDefault) || window.themes.blue;
...
function updateThemeUI(themeName) {
  // Update active theme indicator in theme selector
  ...
  if (themeNameDisplay && window.themes[themeName]) {
    themeNameDisplay.textContent = window.themes[themeName].name;
  }
}
...
function getCurrentTheme() {
  ...
  return window.themes[savedTheme] ? savedTheme : 'blue';
}
...
function initializeThemeManager() {
  ...
  const channel = setupCrossTabListener();
  ...
  if (savedTheme && window.themes[savedTheme]) {
    ...
    // Find the default theme or fall back to blue
    const defaultTheme = Object.entries(window.themes).find(([_, theme]) => theme.isDefault)?.[0] || 'blue';
    applyTheme(defaultTheme, 'local');
  }
